...
type Question = { id: string; question: string; status?: string; answer?: string | null; approved_at?: string | null; answered_at?: string | null; accepted_at?: string | null };
type Gap = { id: string; gap_type?: string; title?: string; description?: string; severity?: string };
...
export default async function ClarificationsPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-lg border border-blue-200 bg-blue-50"><div className="border-b border-blue-200 px-5 py-4"><h2 className="font-semibold text-blue-900">Actions</h2></div><div className="p-5 text-sm"><form action={`/app/tenders/${params.id}/clarifications/run`} method="post"><button type="submit" className="rounded bg-blue-600 px-4 py-2 text-sm font-medium text-white">Run clarifications</button></form><p className="mt-2 text-gray-600 text-xs">Re‑run clarification draft to refresh questions.</p></div></section>
      <section className="grid gap-4 md:grid-cols-2">
        <div className="rounded-lg border border-gray-200 bg-white"><div className="border-b border-gray-200 px-5 py-4"><h2 className="font-semibold text-gray-900">Questions</h2></div><div className="p-5 text-sm">{questions?.length ? (questions as Question[]).map((q: Question) => <div key={q.id} className="py-1"><span className="font-medium">{q.question}</span> — Status: <span className={q.status==='answered'?'text-green-600':q.status==='draft'?'text-gray-600':'text-amber-600'}>{q.status}</span></div>) : <p className="text-gray-500">No questions yet.</p>}</div></div>
        <div className="rounded-lg border border-gray-200 bg-white"><div className="border-b border-gray-200 px-5 py-4"><h2 className="font-semibold text-gray-900">Evaluation gaps</h2></div><div className="p-5 text-sm">{gaps?.length ? (gaps as Gap[]).map((g: Gap) => <div key={g.id} className="py-1"><span className="font-medium">{g.gap_type}</span> — {g.title} <span className="text-gray-400">{g.description}</span> <span className={g.severity==='critical'?'text-red-600':g.severity==='warning'?'text-amber-600':'text-gray-600'}>{g.severity}</span></div>) : <p className="text-gray-500">No gaps.</p>}</div></div>
      </section>
    </div>
  );
}
