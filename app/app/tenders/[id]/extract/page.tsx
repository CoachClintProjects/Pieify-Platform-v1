...
export default async function ExtractionReviewPage({ params }: { params: { id: string } }) {
  ...
  if (!sections?.length) {
    ...
    if (docIds.length) {
      ...
      sections = result.sections.map((s, i) => ({ ...s, id: `sim-${i}` })); candidates = result.candidates.map((c, i) => ({ ...c, id: `sim-${i}` })); conflicts = result.conflicts.map((c, i) => ({ ...c, id: `sim-${i}` })); gaps = result.gaps.map((g, i) => ({ ...g, id: `sim-${i}` }));
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <div className="rounded-lg border border-gray-200 bg-white"><div className="border-b border-gray-200 px-5 py-4"><h2 className="font-semibold text-gray-900">Sections</h2></div><div className="p-5 text-sm">{sections?.length ? sections.map(s => <div key={s.id} className="py-1"><span className="font-medium">{s.title}</span> — {s.section_type || '—'} (p.{s.page_start}–{s.page_end}) <span className={s.confidence && s.confidence>0.8?'text-green-600':'text-amber-600'}>confidence {Math.round((s.confidence||0)*100)}%</span></div>) : <p className="text-gray-500">No sections extracted yet.</p>}</div></div>
