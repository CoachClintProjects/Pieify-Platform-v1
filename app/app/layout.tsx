...
const clientNav = [
  ['Home', '/app'], ['Tenders', '/app/tenders'], ['Requirements', '/app/tenders'], ['Questions', '/app/tenders'],
  ['Inventory', '/app/inventory'], ['Companies', '/app/customers'], ['Suppliers', '/app/suppliers'], ['Quotes', '/app/quotes'], ['Documents', '/app/documents'], ['Reports', '/app/reports'],
];
const platformNav = [
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="hs-app">
    ...
    <aside className="hs-side"><Link href="/app" className="hs-brand"><span>PIE</span>ify</Link><div className="hs-context"><div className="hs-context-kicker">{isSuperuser ? 'Owner / Operator' : 'Workspace'}</div><div className="hs-context-name">{isSuperuser ? 'Platform Control Plane' : 'Procurement'}</div></div><nav className="hs-nav">{isSuperuser ? platformNav.map(section => <div key={section.group}><div className="hs-nav-label">{section.group}</div>{section.items.map(([label,href])=><Link key={label} href={href} className="hs-link"><i className="hs-dot"/><span>{label}</span></Link>)}</div>) : <><div className="hs-nav-label">Workspace</div>{clientNav.map(([label,href])=><Link key={label} href={href} className="hs-link"><i className="hs-dot"/><span>{label}</span></Link>)}</>}</nav><div className="hs-bottom"><div className="hs-bottom-email">{user.email}</div><Link href="/auth/signout">Sign out</Link></div></aside>
